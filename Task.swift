...
extension Task {
    ...
    func save() {
        var tasks = Task.getTasks()
                ...
                // 1. Find if the task already exists by idb ...... this is used when we edited already existing task so it delets the the existing one 
                if let index = tasks.firstIndex(where: { $0.id == self.id }) {
// 2. Remove the old version
            tasks.remove(at: index)
// 3. Insert the updated task at the same index
            tasks.insert(self, at: index)
                } else {
// 4. Task not found – append new one
                    tasks.append(self)
                }
